...
export default function EmptyState({ hasFilters }) {
  return (
    ...
    >
      <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-stone-100 to-stone-200 flex items-center justify-center mb-6">
        {hasFilters ? (
          <Newspaper className="w-10 h-10 text-stone-400" />
        ) : (
          <Globe className="w-10 h-10 text-stone-400" />
        )}
      </div>
